...
# this test doesn't even work for regular tibbles...skipping
# test_that("inputs are recycled", {
#   expect_equal(
#     tibble() %>% summarise(x = 1, y = 1:3, z = 1) %>% collect,
#     tibble(x = 1, y = 1:3, z = 1)
#   )
#
#   gf <- group_by(spark_tbl(tibble(a = 1:2)), a)
#   expect_equal(
#     gf %>% summarise(x = 1, y = 1:3, z = 1) %>% collect,
...
# this test is also kind of dumb, at least from my limited POV
# test_that("works with empty data frames", {
#   # 0 rows
#   df <- spark_tbl(tibble(x = integer()))
